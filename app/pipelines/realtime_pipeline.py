...
class RealtimePipeline:
    """实时生成 Pipeline - 完全照搬 StreamDiffusion 原始实现"""
    ...
    class InputParams(BaseModel):
        ...
        seed: int = Field(502923423887318, title="Seed", id="seed")
    ...
    def _create_stream(self, params: "RealtimePipeline.InputParams") -> StreamDiffusionWrapper:
        ...
        return StreamDiffusionWrapper(
            ...
            enable_similar_image_filter=self._args.get("enable_similar_image_filter", True),
    ...
    def _prepare_if_needed(self, params: "RealtimePipeline.InputParams") -> None:
        ...
        self.logger.debug("Re-preparing realtime stream")
        self.stream.prepare(**prepare_args)
        self._prepare_cache = prepare_args

    def _ensure_stream(self, params: "RealtimePipeline.InputParams") -> None:
        selection = params.lora_selection or "none"
        if selection == self._active_lora:
            return

        self.logger.info("Switching realtime LoRA selection to %s", selection)
        self.stream = self._create_stream(params)
        self._prepare_cache = {}
        self._active_lora = selection

    def predict(self, params: "RealtimePipeline.InputParams") -> Image.Image:
